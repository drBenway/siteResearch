...
Class PHPCurlCrawler {
     ...
     * @return int
     */
    public function pageStatus($url)
    {
        try {
            $status = $this->httpHeaders($url);
        } catch (Exception $e) {
     ...
     * @return boolean
     */
    public function pageMoved($url)
    {
        $status = $this->pageStatus($url);
     ...
     * @return string
     */
    public function getRedirectLocation($url)
    {
     ...
     *
     */
    public function httpHeaders($url)
    {
    ...
    public function validHtmlPage($url)
    {
        curl_setopt($this->curl, CURLOPT_URL, $url);
        curl_setopt($this->curl, CURLOPT_USERAGENT, $this->default_useragent);
        curl_setopt($this->curl, CURLOPT_HEADER, TRUE);
     ...
     */
    public function postToPage($url, $postvar)
    {
        curl_setopt($this->curl, CURLOPT_URL, "$url");
        curl_setopt($this->curl, CURLOPT_POST, 1);
